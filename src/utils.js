...
/**
 * Temporarily replaces keywords in the input string before Sass compilation,
 * and provides a function to restore them in the output CSS.
 * This is a custom feature for sass-multi-version-loader.
 *
 * @param {string[]} ignoreKeywords - Array of keywords to temporarily replace
 * @param {string} data - The Sass/SCSS content
 * @returns {{ processedData: string, replaceBack: (css: string) => string }}
 */
function temReplaceKeywordsInString(ignoreKeywords, data) {
  ...
  if (ignoreKeywords.length > 0) {
    ...
    processedData = ignoreKeywords.reduce((acc, keyword, i) => {
  ...
  // Return processed data and a function to restore keywords
  const replaceBack = (css) => {
    if (isReplaceDirty) {
      // Replace the placeholders back to the original keywords
      return replaceKeywords.reduce(
        (acc, keyword, i) =>
          acc.replace(new RegExp(keyword, "g"), ignoreKeywords[i]),
        css,
      );
    }
    return css;
  ...
  return { processedData, replaceBack };
}
