 ...
 * This fork adds temReplaceKeywordsInString functionality for keyword replacement.
...
async function loader(content) {
  ...
  // Get ignoreKeywords option for temReplaceKeywordsInString functionality
  const ignoreKeywords = options.ignoreKeywords || [];

  // Apply temReplaceKeywordsInString - temporarily replace keywords before compilation
  const { processedData, replaceBack } = temReplaceKeywordsInString(
    ignoreKeywords,
    sassOptions.data,
  );

  // Update sassOptions with processed data
  sassOptions.data = processedData;
  ...
  // Apply replaceBack to restore the original keywords in CSS output
  const css = replaceBack(result.css.toString());
